...
MongoClient.connect('mongodb://localhost:27017', function(err, client){
	...
	db.command({dropDatabase: 1}, function(err){
		clog.info("test DB 삭제.");
		db.member = db.collection('student');  // 'student' collection 생성
		clog.info('DB 초기화 완료.');
	...
	var shops = [
		{
			...
			etc: "기타 정보 없음.",
			couponQuantity: 1,
			picture: {file: "shop_01.jpg", desc: "을지로 골뱅이 외부 모습"}
		},
		{
			_id: shopObjId[1],
			shopName: "스타벅스 압구정", 
			...
			etc: "압구정 최고의 스타벅스입니다.",
			couponQuantity: 1,
			picture: {file: "shop_02.jpg", desc: "스타벅스 압구정 전경"}
		},
		{
			_id: shopObjId[2],
			...
			couponQuantity: 1,
			picture: {file: "shop_03.jpg", desc: "투썸 플레이스 압구정 전경"}
		}	     
	];
...
function registCoupon(){
    ...
    // 4. 쿠폰 데이터 등록 완료 시 후기 데이터 등록
		db.coupon.insert(coupons, registEpilogue);
